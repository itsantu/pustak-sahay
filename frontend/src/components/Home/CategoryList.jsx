...
const ShopByCategory = () => {
  return (
    <div className="px-10 py-14 border-b border-gray-600">
      ...
      <div className="grid grid-cols-4 lg:grid-cols-8 gap-3 md:gap-6 justify-center">
        {[
          ...
          <Link to={`/books/${category.label.trim().toLowerCase()}`}
            key={index}
            className="flex flex-col items-center text-center p-1 md:p-4 bg-gray-100 rounded-lg shadow-md transition-transform duration-200  hover:scale-105 hover:transform hover:-translate-y-2 hover:shadow-2xl cursor-pointer"
          >
            <div className="text-xl md:text-4xl ">{category.icon}</div>
            <p className="text-[0.7rem] md:text-lg font-semibold mt-2">{category.label}</p>
          </Link>
        ))}
      </div>
    </div>
